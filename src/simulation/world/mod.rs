...
impl Plugin for WorldPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.configure_loading_state(
            LoadingStateConfig::new(AssetLoadingState::Loading).load_collection::<LevelAsset>(),
        );
...
fn load_world(mut commands: Commands, level: Res<LevelAsset>) {
...
fn add_world_cell_colliders(mut commands: Commands, cell_query: Query<Entity, Added<IntGridCell>>) {
    let mut count = 0;
    cell_query.for_each(|entity| {
        count += 1;
        commands.entity(entity).insert(Collider::cuboid(16., 16.));
    });
